...
def load_data(file_path):
    ...
    required_cols = ['PaperID', 'Actual_C5_log', 'Predicted_C5_log']
...
def calculate_statistics(df):
    ...
    # Calculate R-squared
    r_squared = stats.pearsonr(df['Actual_C5_log'], df['Predicted_C5_log'])[0] ** 2
    
    stats_dict = {
        'Total Papers': len(df),
        'Zero Citation Papers': (df['Actual_C5_log'] == 0).sum(),
        'Zero Citation Papers (%)': (df['Actual_C5_log'] == 0).mean() * 100,
        'Mean Actual Citations': df['Actual_C5_log'].mean(),
        'Median Actual Citations': df['Actual_C5_log'].median(),
        'Max Actual Citations': df['Actual_C5_log'].max(),
        'Mean Predicted Citations': df['Predicted_C5_log'].mean(),
        'Median Predicted Citations': df['Predicted_C5_log'].median(),
        'Max Predicted Citations': df['Predicted_C5_log'].max(),
        'Correlation': df['Actual_C5_log'].corr(df['Predicted_C5_log']),
        'R-squared': r_squared,
        'Mean Absolute Error': (df['Actual_C5_log'] - df['Predicted_C5_log']).abs().mean(),
        'Median Absolute Error': (df['Actual_C5_log'] - df['Predicted_C5_log']).abs().median(),
        'Root Mean Square Error': np.sqrt(((df['Actual_C5_log'] - df['Predicted_C5_log']) ** 2).mean()),
        'Accuracy (within ±10)': (abs(df['Actual_C5_log'] - df['Predicted_C5_log']) <= 10).mean() * 100
    }
...
def print_statistics(stats_dict):
    ...
    print(f"R-squared: {stats_dict['R-squared']:.3f}")
...
def analyze_extreme_cases(df):
    ...
    # Calculate error
    df['Error'] = df['Predicted_C5_log'] - df['Actual_C5_log']
    ...
    for _, row in largest_over.iterrows():
        print(f"Paper ID: {row['PaperID']}, Actual: {row['Actual_C5_log']:.1f}, "
              f"Predicted: {row['Predicted_C5_log']:.1f}, Error: {row['Error']:.1f}")
...
def create_binned_data(df, max_bin=50):
    ...
    for _, row in df.iterrows():
        ...
        bins[bin_key]['total_predicted'] += row['Predicted_C5_log']
        bins[bin_key]['min_predicted'] = min(bins[bin_key]['min_predicted'], row['Predicted_C5_log'])
        bins[bin_key]['max_predicted'] = max(bins[bin_key]['max_predicted'], row['Predicted_C5_log'])
...
def create_visualizations(df, bin_df, output_dir='plots_log_scale'):
    ...
    # Create scatter plot
    ax = sns.scatterplot(x='Actual_C5_log', y='Predicted_C5_log', data=sample_df, alpha=0.6)
    
    # Add perfect prediction line
    perfect_line = np.linspace(0, 10, 100)
    plt.plot(perfect_line, perfect_line, 'r--', label='Perfect Prediction')
    
    # Set x and y limits to 10 as requested
    plt.xlim(-1, 10)
    plt.ylim(-1, 10)
